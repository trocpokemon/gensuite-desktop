...
import { BookmarkCheck, Plus, Save, Trash2 } from 'lucide-react';
...
export function SubtitleDesigner({ config, onChange, ratio, backgroundPath, backgroundIsVideo }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-5"><ColorField label="Màu chữ" value={config.textColor} onChange={(textColor) => update({ textColor })} /><ColorField label="Màu highlight" value={config.highlightColor} onChange={(highlightColor) => update({ highlightColor })} /><ColorField label="Màu nền" value={config.backgroundColor} onChange={(backgroundColor) => update({ backgroundColor })} /><ColorField label="Màu viền" value={config.outlineColor} onChange={(outlineColor) => update({ outlineColor })} /></div>
