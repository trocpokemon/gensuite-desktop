...
export function getScriptProvider(engine: ScriptEngine, keys: AppSettings, model?: string, feature?: GenSuiteFeature): IScriptProvider {
  switch (engine) {
    case 'gensuite':
      return new GenSuiteScriptAdapter(model?.trim() || undefined, feature);
    case 'gemini':
    default:
