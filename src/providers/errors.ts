...
const STRUCTURED_ERROR_MESSAGES: Record<AppErrorCode, (error: PublicAppError) => string> = {
  ...
  TRANSCRIPTION_INSTALLATION_INCOMPLETE: () => 'Bộ cài nhận dạng đang thiếu tệp hoặc có tệp bị hỏng. Hãy cài đè bằng bản Setup đầy đủ rồi thử lại.',
  TRANSCRIPTION_SYSTEM_SUPPORT_MISSING: () => 'Máy đang thiếu gói hỗ trợ Windows cần cho nhận dạng. Hãy cài hoặc sửa chữa gói hỗ trợ Microsoft, khởi động lại máy rồi thử lại.',
  TRANSCRIPTION_COMPONENT_QUARANTINED: () => 'Công cụ bảo mật đã cách ly một tệp cần cho nhận dạng. Hãy mở Lịch sử bảo vệ, cho phép tệp của GenSuite rồi cài lại ứng dụng.',
